...
In this section, we will establish a server template to simulate a real-world corporate environment with a standardised VM template. This approach reduces the chance of errors and saves time. We will use Sysprep to ensure that cloned machines do not share the same Security Identifier (SID). If we create the domain controller and clone it to make a DHCP server, both will have the same SID, preventing us from adding the DHCP server to the domain.
...
We will go to settings, update and security, and make sure to check for any updates to ensure the latest patches are installed.
...
Next, we will use the template called "VM Template" to create a clone named "DC (Domain Controller)." Ensure to select the option to generate a new MAC address and select the option for a full clone.
...
Afterwards, right-click on the DC server, go to **Settings**, navigate to the **Expert** tab, then to general and then advanced to enable Bi-directional Sharing for both the shared clipboard and drag and drop option. This will allow us to drag and drop our PowerShell script onto the DC server. 
...
Start the DC and go through the setup process, choose custom installation and make a password for are example, we use Secure123.

![image](https://github.com/user-attachments/assets/21f05710-32bb-4f81-8809-6ae6aecebd2b)


After this, we will download the guest addition of VirtualBox using device options, and we will download and install and restart it. This will allow us to drag and drop our PowerShell script later on.
...
After installation, open Control Panel and navigate to Network and Internet, Network and Sharing Centre, then click Change adapter settings. 
...
Next, we will go on to Search and look for search advanced system properties, and then we go to the computer name and then change and change it to DC and then press ok and then restart.
...
Next, we will be downloading RAS/NAT, and this will act as a gateway for our internal network. We will go to server manager and add roles and sevices lick 'Role-based or feature-based installation and Select our server from the server pool  And we will go next and till we get to services and we will chose remote access services and we will press next till we get to the we to the options and wil choose routing choose add options and press next until we get to install. After the installation completes, we can close it.  
...
Go to the tools on the server manager and then on routing and remote services. Once we are on we right click and press configure and the we right click new som and we should see option for routing with nat and we will choose that and then will will use the public lab external ip interface for the nat and press next until we get to the end refresh by rigth clicking and wes should see a green arrow.
...
NOTE: If you don't see the networks that we configured, restart the routing and remote access wizard:
...
Then press next, and the output will be created. 

![image](https://github.com/user-attachments/assets/f5cb4937-d1b4-4b8d-bc9f-0388f94a40ca)


We can right click on it to create a new and there will option for a new user. We will name our user Ahmed Shego, and then will use the name username a-ashego. This is similar to what is used in real life, and we will go to the next. 
...
Next, we will right click and go to settings and then to network and change our adaptor from NAT to internal and then press ok.
![image](https://github.com/user-attachments/assets/3a05b6bb-4f2c-433d-bfb4-bc2065c11b13)
After going through the installation process, which will be the same as the DC server, we will also use the same password for the administrator
![image](https://github.com/user-attachments/assets/81729613-c71c-48ae-a6c0-c716fcb33f53)

After installation, open Control Panel and navigate to Network and Internet, Network and Sharing Centre, then click Change adapter settings. Right click on the adaptor choose Properties, double-click Internet Protocol Version 4 (TCP/IPv4), select “Use the following IP address,” and enter the address from your network diagram. Click OK to save.
...
Just like we did before, we will go to the Server Manager Dashboard, Add roles and features, Role-based or feature-based installation, Select our server, Select DHCP and add features and then install.
...
We will not be setting any exclusions, and we can leave the lease on 8 days.
...
After starting up the Win10 Client VM and going through the installation process, which will be similar to the DC and DHCP server.
![image](https://github.com/user-attachments/assets/0ffc4c8d-9e1c-43d9-8f08-3462a2043e38)
Once it restarts, make sure to use the following options. I don't have internet. 'Continue with limited setup
...
Create a username and password that will be used by the User and the same password.  Put in any answer for the security questions and select not now.
...
Use the Domain Admin Account to authorise domain joining. Then restart.
![image](https://github.com/user-attachments/assets/e4b97d2d-ca87-453f-8759-a0d8b41a3ea0)

##  Setting up Group Policies on DC

###  Setting Shared Folder

First, create a folder on the DC Local Disk called shared. Right-click it, choose Properties, then go to the Sharing tab, Advanced Sharing. Remove Everyone, add Domain Users, and grant them Read permission and apply and press ok.
...
Create & link a GPO named Restrict-Registry. Make sure to right-click and choose Enforce.
...
We should have Four Group policies overall
...
Go to the Client and Sign in with the User Account. In my example, I will sign in with lsmith. We will go to cmd and then type gpupdate /force, then we restart and check each policy to see if it works.
